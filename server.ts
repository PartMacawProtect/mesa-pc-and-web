...
// Enable CORS for Electron desktop client and external requests
...
interface ServerUserProfile {
  ...
  privateKey?: string; // Stringified JWK or export string
}
...
// Map of userEmail (lowercase) -> Backup of Profile Details (avatar, bio, statusMessage) on Account Deletion
const deletedProfilesBackup = new Map<string, { username: string; avatar?: string; bio?: string; statusMessage?: string; publicKey?: string; privateKey?: string }>();
...
function loadDatabase() {
  try {
    if (fs.existsSync(DB_FILE)) {
      ...
      if (data.deletedProfilesBackup && Array.isArray(data.deletedProfilesBackup)) {
        data.deletedProfilesBackup.forEach(([k, v]: any) => {
...
async function sendOTPEmail(to: string, code: string, type: "register" | "reset"): Promise<{ success: boolean; isMock: boolean; error?: string }> {
  // Read SMTP settings strictly from environment variables to keep them 100% hidden from source files and client builds
...
// Fetch full user profile details for a specific email
...
app.post("/api/users/pulse", (req, res) => {
  const { email, username, avatar, statusMessage, bio, publicKey, privateKey, isUpdate } = req.body;
  if (!email) {
  ...
  // Update profile details dynamically if they are passed as parameters and isUpdate is true
...
app.post("/api/users/delete-account", (req, res) => {
  ...
  // Remove user completely from any backups and state
  deletedProfilesBackup.delete(normalizedEmail);
...
// Endpoint to dynamically bundle the desktop-client folder into a download zip
app.get("/api/download/desktop-zip", async (req, res) => {
  try {
    const zip = new JSZip();
    const folderPath = path.join(process.cwd(), "desktop-client");
    const requiredFiles = ["package.json", "main-electron.cjs", "preload-electron.js", "README.md", ".env.example"];
    ...
    // Dynamic generation of .env file inside ZIP so it references the download origin
    let envContent = "";
    const originalEnvPath = path.join(folderPath, ".env");
    if (fs.existsSync(originalEnvPath)) {
      envContent = fs.readFileSync(originalEnvPath, "utf-8");
    } else {
      const protocol = req.secure || req.headers["x-forwarded-proto"] === "https" ? "https" : "http";
      const host = req.headers.host || "mesa-pc-and-web-production.up.railway.app";
      const downloadDomain = `${protocol}://${host}`;
      envContent = `# Mesa Desktop Client Environment Configuration\n# Auto-generated dynamically on download!\nAPP_URL=${downloadDomain}\n`;
    }
    ...
    res.setHeader("Content-Disposition", "attachment; filename=Mesa-Electron-Client.zip");
    res.send(content);
  } catch (err: any) {
    console.error("Failed to generate desktop-client zip payload:", err);
    res.status(500).json({ success: false, error: "Не удалось сформировать архив клиента." });
  }
});
